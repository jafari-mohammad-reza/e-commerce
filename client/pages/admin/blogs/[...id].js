        ...
        <Fragment>
            {
                isLoading ? <h1>Loading</h1> : (
                    <form className={'flex flex-row w-full h-max flex-wrap px-6 md:px-12 lg:px-24 md:justify-between py-10 items-center'} encType="multipart/form-data" onSubmit={(e) => handleSubmit(e)}>
                        <input type="text" className={'admin_input'} placeholder={'blog title....'} required={true} ref={titleRef}/>
                        <input type="text" className={'admin_input'} placeholder={'blog overView....'} required={true} ref={overViewRef}/>
                        <input type="text" className={'admin_input'} placeholder={'blog tags....'} required={true} ref={tagsRef}/>
                        <textarea rows={10} cols={15} className={'w-full my-10 mx-5 bg-sky-50  border-0 outline-none transition-all duration-100 shadow  shadow-3xl focus:border-b-4 border-gray-600 py-6 px-12 rounded-2xl'} placeholder={'blog content'} required={true} ref={contentRef}/>
                        <select name="parent"  className={'admin_input'} ref={categoryRef}>
                            <option value={undefined} disabled={false} defaultChecked={true}>Select One category</option>
                            {allCategories && allCategories.map(category => (
                                <option  value={category._id} key={category._id}>{category.title}</option>
                            ))}
                        </select>
                        <input type="file"  className={' admin_input'}  required={true} onChange={(e) => setImage(e.target.files)}/>
