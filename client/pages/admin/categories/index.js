               ...
               <table className="w-full text-sm text-left text-gray-500     ">
                   ...
                   {data.map(category => (
                       <tr key={category._id}  className="bg-white border-b text-lg md:text-3xl ">
                           <td scope="row" className="py-4 px-6 text-3xl font-bold text-gray-900 whitespace-nowrap ">{category.title}</td>
                           <td className="py-4 px-6">{category.parent ? category.parent : "🚫"}</td>
                           <td className="py-4 px-6 max-w-min ">
                                    <Image src={category.imageURL} alt={"🚫"} width={120} height={120} layout={'fixed'} objectFit={"contain"}/>
                           </td>
