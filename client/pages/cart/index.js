...
import {GetUserShoppingCart_Query} from "../../graphql/Queries/GlobalQueries";
...
const Index = ({cartData}) => {
    ...
    return (
        <div
            className={'w-full h-full  md:h-[87.5vh] overflow-hidden grid grid-cols-1 md:grid-cols-12  px-6 md:px-12 lg:px-24'}>
            <div className={'md:col-span-8 lg:col-span-9  h-full overflow-y-scroll '}>
                {
                    data ? (
                        data.map(product => (
                            ...
                            <div key={product._id}
                                 className={'w-full md:w-3/4 flex items-center justify-between px-10 mb-5 py-7   border-b-2 border-b-blue-500'}>
                                ...
                                <AiOutlinePlus className={'w-12 h-12 bg-blue-500 text-white rounded-full '}
                                               onClick={() => {
            ...
            <div
                className={'md:col-span-4 lg:col-span-3  h-full lg:h-[90%] overflow-y-scroll w-full rounded-3xl bg-blue-600 text-white px-12 py-8 '}>
                ...
                <div className={'flex items-start flex-col justify-center space-y-4'}>
                    {data && data.map(product => (
                        <div key={product._id}
                             className={'w-full h-max flex items-center justify-around  pb-5 border-b-2 border-b-black'}>
                            ...
                            <button className={'font-bold text-5xl rounded-3xl bg-red-500 text-gray-50 w-max'}
                                    onClick={() => {
                                        removeItemFunction({
                                            variables: {
                                                productId: product._id
                    ...
                    <button
                        className={'bg-white text-blue-500 font-bold text-2xl py-5 px-8 rounded-3xl absolute self-center fixed bottom-48 transition hover:scale-110  '}>Checkout
                    </button>
                </div>
            </div>
        </div>
