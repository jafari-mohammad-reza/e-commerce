...
const directoryCreator = (req) => {
    const date = new Date();
    const currentDate = `${date.getFullYear().toString()}\\${date.getMonth().toString()}\\${date.getDay().toString()}`
    const fileUploadPath = path.join(__dirname, "..", "..", "public", "uploads", "blogs", currentDate)
...
const imageUploader = multer({storage , fileTypeFilter , limit : {fileSize : imageMaxSize}})
function deleteImageFromPath(path){
    if(fs.existsSync(path.join(__dirname, "..", "..", "public",  path))){
        fs.unlinkSync(path)
    }
}
module.exports = {imageUploader , deleteImageFromPath}